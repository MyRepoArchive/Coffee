...
module.exports = (req, res) => {
  ...
  const sql = `DELETE FROM commands WHERE ${props.map(property => {
  ...
  db.query(sql, (err, result) => {
    ...
    const deleted = cache.commands.length ?
      (
        props.map(prop => {
          return cache.commands.filter(command => properties[prop].includes(command[prop]));
        })
    ...
    if (deleted) deleted.forEach(deleted => cache.commands.splice(cache.commands.findIndex(command => command.id === deleted.id), 1));

    return res.json({ result, sql, commands: deleted });
  });
};