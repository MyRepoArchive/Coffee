...
module.exports = {
  name: "loja",
  aliases: ["shop", "compras", "shopping", "store", "perigo"],
  type: "Economia",
  ...
  async execute(message, args, comando, client, prefix, connection) {
    ...
    collector1.on("collect", async (reaction, user) => {
      ...
      if (emojiArray.includes(reaction.emoji.identifier) && reaction.message.embeds[0].title === 'Selecione o tipo de produto que deseja comprar!') {
        ...
        if (distinctTypes[emojiArray.indexOf(reaction.emoji.identifier)] === 'score-style') {
        ...
        } else if (distinctTypes[emojiArray.indexOf(reaction.emoji.identifier)] === 'rate-reducer') {

          const itensRateReduces = getStoreItems.result.filter(x => x.type === distinctTypes[emojiArray.indexOf(reaction.emoji.identifier)])
          const primeiroItem = itensRateReduces[0]
          ...
          collector2.on('collect', async (reaction, user) => {
            ...
            if (reaction.emoji.identifier === emojis.fastforward && reaction.message.embeds[0].title.startsWith(`Deseja comprar o redutor`)) {
              const page = Number(reaction.message.embeds[0].title.split('(')[reaction.message.embeds[0].title.split('(').length - 1].split('/')[0])
              const item = itensRateReduces[page]

              const embedPage = new Discord.MessageEmbed()
                .setTitle(`Deseja comprar o redutor **${item.name}**? (${page + 1}/${itensRateReduces.length})`)
                .setThumbnail(`https://github.com/JoaoSCoelho/Coffe/blob/master/image/emojis/business-money.png?raw=true`)
                .setColor(message.guild.me.displayHexColor)
                .setAuthor(message.author.username, message.author.displayAvatarURL())
                .setTimestamp()
                .setFooter(`Sistema de economia ${client.user.username}`, client.user.displayAvatarURL())
              page + 1 >= itensRateReduces.length ?
                embedPage.setDescription(`<:${emojis.rewind}> Redutor anterior\n<:${emojis.linebankbusinesscard}> Comprar redutor - <:${emojis.linecoinbitcoin}>**${item.price}**\n<:${emojis.linebankcard}> Comprar redutor (global) - <:${emojis.linebitcoinmoney}>**${item.global_price}**\n\n**${item.description}**`) :
                embedPage.setDescription(`<:${emojis.fastforward}> Próximo redutor\n<:${emojis.rewind}> Redutor anterior\n<:${emojis.linebankbusinesscard}> Comprar redutor - <:${emojis.linecoinbitcoin}>**${item.price}**\n<:${emojis.linebankcard}> Comprar redutor (global) - <:${emojis.linebitcoinmoney}>**${item.global_price}**\n\n**${item.description}**`)

              msg.edit(embedPage)

              if (page + 1 >= itensRateReduces.length) msg.reactions.cache.find(react => react.emoji.identifier === emojis.fastforward).users.remove(client.user.id)

              msg.react(emojis.rewind)
            } else if (reaction.emoji.identifier === emojis.rewind && reaction.message.embeds[0].title.startsWith(`Deseja comprar o redutor`)) {
              const page = Number(reaction.message.embeds[0].title.split('(')[reaction.message.embeds[0].title.split('(').length - 1].split('/')[0])
              const item = itensRateReduces[page - 2]
              ...
              msg.react(emojis.fastforward)
            } else if (reaction.message.embeds[0].title.startsWith(`Deseja comprar o redutor`)) {
              const redutorName = reaction.message.embeds[0].title.split('**')[1]
              const redutor = itensRateReduces.find(x => x.name === redutorName)
              ...
              const sucessBuyEmbed = new Discord.MessageEmbed()
                ...
                .setFooter(`Sistema de economia ${client.user.username}`, client.user.displayAvatarURL())
                .setTimestamp()
                .setTitle(`<:${emojis.circlecheckverde}> Compra realizada com sucesso! \`${redutor.name}\``)
                .setThumbnail(`https://github.com/JoaoSCoelho/Coffe/blob/master/image/emojis/business-money.png?raw=true`)
              ...
              if (reaction.emoji.identifier === emojis.linebankbusinesscard) {
                ...
                if (redutor.price > authorMoney) return run(message, client, `<:${emojis.alertcircleamarelo}> ${message.author}, você não possui <:${emojis.linecoinbitcoin}>CCoins o suficiente para comprar este redutor de taxa!`)

                connection.query(`Select reducao_taxa from score_per_server where userid = '${message.author.id}' and serverid = '${message.guild.id}'`, async (err, result) => {
                  if (err) throw err;
                  const reducaoTaxaAtual = result[0].reducao_taxa
                ...
                if (redutor.global_price > authorBankMoney) return run(message, client, `<:${emojis.alertcircleamarelo}> ${message.author}, você não possui <:${emojis.linecoinbitcoin}>CCoins no banco o suficiente para comprar este redutor de taxa!`)

                connection.query(`Select reducao_taxa from users where iduser = '${message.author.id}'`, (err, result) => {
                  if (err) throw err;
