...
module.exports = {
  ...
  async execute(message, args, comando, client, prefix, connection) {
    ...
    const embed = new Discord.MessageEmbed()
      .setColor(hex.blue2)
      .setTitle(`Lista de emojis (1/${parseInt(emojisArray.length/(2000/maior))})`)
      .setAuthor(message.author.username, message.author.displayAvatarURL())
      .setDescription(`<:${emojisArray.slice(0, 2000/maior).join('> <:')}>`)
    const msg = await message.channel.send(embed)
    if(parseInt(emojisArray.length/(2000/maior)) > 1) {
    ...
    collector.on('collect', (reaction, user) => {
      ...
      if(reaction.emoji.identifier === emojis.fastforward) {
        const embedForward = new Discord.MessageEmbed()
          .setColor(hex.blue2)
          .setTitle(`Lista de emojis (${page+1}/${parseInt(emojisArray.length/(2000/maior))})`)
          .setAuthor(message.author.username, message.author.displayAvatarURL())
          .setDescription(`<:${emojisArray.slice(2000/maior*page, 2000/maior*(page+1)).join('> <:')}>`)
        msg.edit(embedForward)
        msg.react(emojis.rewind)
        if(page+1 === parseInt(emojisArray.length/(2000/maior))) msg.reactions.cache.find(react => react.emoji.identifier === emojis.fastforward).users.remove(client.user)
      }
