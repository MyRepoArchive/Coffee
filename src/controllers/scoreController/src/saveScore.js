...
module.exports = async (members) => {
  ...
  api.get('/inventory', { body: { joinProducts: true } })
    .then(response => {
      ...
      Object.keys(scores).forEach(serverId => {
          ...
          Object.keys(scores[serverId])
            .forEach(userId => {
              if (serverMembers.map(member => member.user).includes(userId)) {
                needCreate.push({ server: serverId, user: userId, score: scores[serverId][userId] });
              } else {
                const member = members.find(member => member.user === userId && server === serverId);
                ...
                const value = scores[serverId][userId] * multiplier + member.score;
  
                needUpdate.push({ id: member.id, value });
              };
            });
        } else {
          Object.keys(scores[serverId]).forEach(userId => needCreate.push({ server: serverId, user: userId, score: scores[serverId][userId] }));
        };
      });
