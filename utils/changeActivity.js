  ...
  async run(intervalActivity, client, Data) { // Função que muda o que o bot exibe no "Activity" a cada 20 segundos
    ...
    intervalActivity = setInterval(() => { // É iniciado o interval
      switch (activityId) { // Para cada activityId é executado um setActivity() diferente e adicionado +1 no activityId
        ...
        case 3:
          const Hora = new Date
          client.user.setActivity(`Hora ${pad.pad((Hora.getUTCHours() < 3) ? Hora.getUTCHours() + 21 : Hora.getUTCHours() - 3, 2)}:${pad.pad(Hora.getUTCMinutes(), 2)}`, { type: "STREAMING", url: "https://github.com/joaoscoelho/Coffe" });
          activityId = 0;
          break;
        default:
          client.user.setActivity(`Estou em ${client.guilds.cache.size} servidores`, { type: "STREAMING", url: "https://github.com/joaoscoelho/Coffe" });
